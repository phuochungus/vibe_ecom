...
func SeedDemoData(db *sql.DB) error {
	...
	if err := db.QueryRow(`SELECT COUNT(*) FROM products WHERE deleted_at IS NULL`).Scan(&productCount); err != nil {
		return fmt.Errorf("count products: %w", err)
	}
	if productCount == 0 {
		if _, err := db.Exec(
			`INSERT INTO products (id, sku, name, description, price_cents, stock, status, created_at, updated_at)
			 VALUES (?, 'CLUB-001', 'Driver X', '', 159000000, 20, 'ACTIVE', ?, ?),
			        (?, 'CLUB-002', 'Putter Pro', '', 89000000, 15, 'ACTIVE', ?, ?)`,
